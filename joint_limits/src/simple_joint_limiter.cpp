  ...
  trajectory_msgs::msg::JointTrajectoryPoint & desired_joint_states, const rclcpp::Duration & dt)
{
  ...
  for (auto index = 0u; index < num_joints; ++index)
  {
    ...
    {
      auto pos = std::max(std::min(joint_limits_[index].max_position, desired_pos[index]), joint_limits_[index].min_position);
    ...
    desired_accel[index] = (desired_vel[index] - current_joint_states.velocities[index]) / dt_seconds;
    ...
    if (joint_limits_[index].has_acceleration_limits)
    {
      ...
      {
        desired_accel[index] = std::copysign(joint_limits_[index].max_acceleration, desired_accel[index]);
        desired_vel[index] = current_joint_states.velocities[index] + desired_accel[index] * dt_seconds;
    ...
    if (joint_limits_[index].has_position_limits)
    {
      ...
      // coming to a rest.
      double stopping_accel = joint_limits_[index].has_acceleration_limits ? joint_limits_[index].max_acceleration :
                                                                     std::abs(desired_vel[index] / dt_seconds);
      double stopping_distance = std::abs(
        (-desired_vel[index] * desired_vel[index]) / (2 * stopping_accel));
    ...
    for (auto index = 0u; index < num_joints; ++index) 
    {
      ...
      if (joint_limits_[index].has_acceleration_limits)
      {
        desired_accel[index] = std::copysign(
          std::min(std::abs(desired_accel[index]), joint_limits_[index].max_acceleration), desired_accel[index]);
      }
      ...
      desired_pos[index] =
        ...
        current_joint_states.velocities[index] * dt_seconds + 0.5 * desired_accel[index] * dt_seconds * dt_seconds;
    }
  }
