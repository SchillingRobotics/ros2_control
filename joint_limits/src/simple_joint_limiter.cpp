  ...
  trajectory_msgs::msg::JointTrajectoryPoint & desired_joint_states, const rclcpp::Duration & dt)
{
  ...
  for (size_t index = 0; index < number_of_joints_; ++index)
  {
    ...
    if (has_pos_cmd)
    {
      ...
      if (std::fabs(position_difference) > VALUE_CONSIDERED_ZERO &&
          std::fabs(desired_vel[index]) <= VALUE_CONSIDERED_ZERO)
      {
        // if (index == 0)
        //   RCLCPP_WARN(
        //     node_logging_itf_->get_logger(), "Calculate desired velocity: (%.5f - %.5f) / %.5f", desired_pos[index], current_joint_states.positions[index], dt_seconds);
    ...
    if (joint_limits_[index].has_velocity_limits)
    {
      ...
      if (std::fabs(desired_vel[index]) > joint_limits_[index].max_velocity)
      {
        ...
        // if (index == 0)
        // RCLCPP_WARN(
        //   node_logging_itf_->get_logger(), "Desired velocity before pos is: %.5f", desired_vel[index]);
